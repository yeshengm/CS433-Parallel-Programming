...
void computeForce(struct body b, struct QuadTree *tree, double *fx, double *fy) {
	...
	if (isLeaf(tree) && tree->massCenter.x == b.x && tree->massCenter.y == b.y) {
		...
		if (tree->pNorthEast != NULL) computeForce(b, tree->pNorthEast, fx, fy);
		if (tree->pSouthWest != NULL) computeForce(b, tree->pSouthWest, fx, fy);
	}
}
...
void position_step(struct world *world, double time_res, int thread_count) {
	...
	# pragma omp parallel num_threads(thread_count) 
	{
		...
		for (int loc_i = 0; loc_i < loc_num_bodies; ++loc_i) {
			computeForce(world->bodies[loc_i+loc_base], world->tree,
					&force_x[loc_i+loc_base], &force_y[loc_i+loc_base]);
		}
